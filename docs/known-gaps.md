...
| 52 | **A suppressed amendment row no longer contributes to `refs_seen`.** Found while tracing `parse()`'s row loop, not anticipated by the design. | `refs_seen`'s own docstring (`ImportBatch`, `importers/base.py`) says every account ref is included "whether or not the row went on to become a fill or cash movement," derived from the raw rows for exactly that reason. The `continue` for a row in `plan.suppressed`, in `parse()`'s materialised-row loop (`importers/fidelity.py`), sits above the `refs_seen.add(account)` call, so a suppressed original or cancel no longer reaches it — narrowing that invariant. **This is a narrowing with no observed effect, not a bug:** the row it skips is one leg of a netted cluster, and the correction row that survives it is never suppressed, runs through the ordinary path, and adds the same account to `refs_seen` itself. Recorded because the docstring states an unconditional rule and this makes it conditional, not because any account currently goes unrepresented. |
