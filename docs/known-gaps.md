...
| 26 | Two different behaviours for an unknown account id (was three) | **The `snapshot add` third of this row is FIXED, 2026-08-10.** `cmd_snapshot_add` now runs the same `get_account`-then-check-`None` refusal `cmd_reconcile` uses, before `add_snapshot` is called at all (`cmd_snapshot_add`'s unknown-account refusal in `cli.py`), so an unknown id exits 2 with a message naming it instead of escaping as a raw `asyncpg.ForeignKeyViolationError` traceback past a `main()` that catches only `OSError`. Pinned by `test_snapshot_add_refuses_an_unknown_account` (`tests/db/test_cli.py`). **What remains is a two-way inconsistency, not three:** `reconcile` refuses cleanly, exit 2 (deliberate, spec §8; `cmd_reconcile`'s unknown-account refusal in `cli.py`) and `snapshot add` now matches it, while `positions` still prints "no open positions" (`cli.py:736`) for a well-formed but nonexistent UUID. The resolution direction is unchanged from #22 — make `positions` stricter, do not loosen the other two — and unchanged in status: still open, still not done here. This subsumes and extends #22 above; cross-reference the two rather than treating them as unrelated. |
...
| 31 | ~~**`TRANSFER OF ASSETS ACAT` is the one verb left from this gap's original list.**~~ — **CLOSED 2026-08-19** (`feat/acat-transfer-out`, branch B). The share leg becomes an `asset_transfer` row and the residual cash a `transfer_out` cash movement (both written by `commit_batch` with content-hash dedupe); grouping closes the position at average cost so realised P&L is zero by construction, and the basis leaves with the shares. Verified by `tests/test_fidelity_real_files.py`: every real export now parses with zero blocking rows, and the previously-blocked account's file chain commits end to end. Inbound-shaped rows still refuse the file by design (branch B spec D2/D6). History below kept for the record. Narrowed 2026-08-18 — the list this gap used to name has been closed piece by piece: `MERGER`, `REVERSE SPLIT` and `NAME CHANGED` by gap #33's corporate-action recognition (2026-08-17); `DISTRIBUTION` and the `BUY CANCEL OPENING TRANSACTION`/`CANCELLED TRADE`/`CORRECTED CONFIRM` amendment cluster by `feat/importer-blocking-verbs` (2026-08-18); `IN LIEU OF` was already ordinary cash. Only `TRANSFER OF ASSETS ACAT` still blocks, and it does so alone — verified by running `FidelityImporter.parse()` over all eleven real history files. | `reject()` (`importers/fidelity.py`) blocks any row that both parsed a valid date and carries a nonzero quantity or amount, and an ACAT row's quantity is nonzero, so it blocks. It is deliberately not folded into this same fix: an outbound transfer closes the position with zero realised P&L, because the cost basis leaves with the shares, and modelling that needs a ledger concept this schema does not have — `cash_movement.kind`'s CHECK constraint admits no transfer kind, and `derived_fill` is bound to corporate actions by construction (both `corporate_action_id` and `derived_from_fill_id` `NOT NULL`), so a transfer cannot reuse either table without a migration. It is a new design (branch B), not a rule addition. `cmd_import` (`cli.py`) still refuses the whole import with exit code 2 the moment the row appears — the safe failure, not a corruption risk, same as before. **Corrected 2026-08-18:** this entry used to cite `importers/fidelity.py:298-321` for `reject()` and `cli.py:314-325` for the exit-2 refusal; neither line range still points at the code it named, which is why this entry now cites by name only. |
...
> **Addendum, 2026-08-17 (`feat/import-corporate-actions`).** #33 below is also now
> closed, narrowly — see "Found while importing corporate actions from broker exports"
> further down, and gap #33's own row for what "closed" does and does not mean. Five of
> the seven rows in the table immediately below are open as of this addendum; two (#33,
> #39) are closed.
...
| # | Gap | Why it matters |
|---|---|---|
| 33 | ~~Corporate actions still cannot be imported.~~ — **FIXED, narrowly, 2026-08-17** (`feat/import-corporate-actions`). Five new `Rule`s give `Outcome.CORPORATE_ACTION` (`importers/fidelity.py`) to the row shapes that used to fall through to the money-carrying-unmapped policy and refuse the whole commit — recognised, producing nothing, and, unlike the old default, **not blocking**. `_group_corporate_actions` (`importers/fidelity.py`) groups rows on the venue's own `#REOR` reorganisation reference — a shared base plus a per-leg trailing digit — falling back to `(ex-date, CUSIP pair)` when no usable reference is present; a group whose row count matches no known action shape is reported as unrecognised rather than coerced (spec §7). `cmd_import` renders one `corporate add` proposal per recognised action (`_print_corporate_action_section`, `cli.py`). **This branch removes the corporate-action class of blocker, and no more.** Verified by parsing every real history export: five of the eleven real files still produce blocking entries, on verbs unrelated to corporate actions — a rollover cash check, an ACAT asset transfer, an early distribution, a cancelled buy, and a plain non-spinoff `DISTRIBUTION` — all still hitting the same money-carrying-unmapped policy this branch left alone for everything but corporate actions. **Gap #31, still open and untouched by this branch, already names several of those verbs.** The two accounts do **not** import end to end from this branch alone. **Correction to this gap's own prior claim, made in passing here because nothing else revisits it:** recognition alone was not the whole fix even for the corporate-action rows themselves. A History-dialect export — the only dialect that contains corporate actions — carries no account column at all, so its rows route only via `import --account <uuid>`, which already existed. Recognition removed the *block*; `--account` was always the routing mechanism, and its help text wrongly claimed Fidelity never needs it — corrected on this branch. | What remains true, deliberately: **nothing is stored.** `cmd_import` never calls `db.corporate.add_action` (D2) — every proposal is text for a human to review and run through `corporate add`, which previews by default and refuses duplicates. See gap #31 above for the unrelated verbs still blocking real files, and the new gaps below (starting at #42) for what proposing-not-storing, and the rest of this branch's scope, still leave open. |
...
| 41 | **The synthetic id hash is not injective in the action.** | `_spinoff_fill_id` hashes only `(parent_fill_id, resulting_instrument_id, ex_date)` (`ledger/corporate.py:32-37`) and not the source instrument, so two spinoff actions that differ only in their source but share a resulting instrument and ex-date mint the same synthetic id for a given parent. The provenance map is first-writer-wins by construction — `if child_id in derived_provenance: continue` (`db/trades.py:264-265`) — so which of the two colliding actions the child gets attributed to is **unspecified**: the two share an ex-date by construction (`ex_date` is inside the hash, so a differing ex-date could not collide in the first place), which means `actions_with_ids_for_instruments`'s `ORDER BY ex_date` fetch (`db/corporate.py`) cannot break the tie between them — it does not distinguish rows that already agree on the column it sorts by. No test in this branch constructs that case — it is recorded as a gap, not a reproduced failure. Disambiguating it means adding `instrument_id` to the hash, which re-mints every existing derived fill's id, since that id is `derived_fill`'s primary key and the `ON CONFLICT` target that keeps a live derived fill's identity stable across regroups (`db/schema.sql:291`, `db/trades.py:311`) — closing this is a migration, not a patch. |

---

## Found while importing corporate actions from broker exports (2026-08-17)

`Outcome.CORPORATE_ACTION` (Task 1's five `Rule`s), `_group_corporate_actions` grouping on
the venue's own `#REOR` reference with an `(ex-date, CUSIP pair)` fallback (Task 2),
ratio derivation cross-checked against the venue's own `N FOR N` text (Task 3), and
`cmd_import`'s rendered `corporate add` proposals (Task 4) close gap #33 above, narrowly:
corporate-action rows no longer block an import. **The two accounts still do not import
end to end** — gap #31, untouched by this branch, still blocks five of the eleven real
history files on verbs unrelated to corporate actions (a rollover cash check, an ACAT
asset transfer, an early distribution, a cancelled buy, a plain non-spinoff
`DISTRIBUTION`), all under the same money-carrying-unmapped policy this branch left alone
for everything but corporate actions. Full design:
[`docs/superpowers/specs/2026-08-17-importing-corporate-actions-design.md`](superpowers/specs/2026-08-17-importing-corporate-actions-design.md).
Its own §10 names five gaps this design creates by its own decisions — D2's
propose-not-store, D6's cash-in-lieu, D7's advisory CUSIP resolution, D4's
venue-specific grouping, and the incremental-import problem its own §1 verification
table found this branch does not solve. A sixth was found only once the ledger-completion
code (Task 4's spinoff-ratio read) was actually written; the design did not anticipate it.

| # | Gap | Why it matters |
|---|---|---|
| 42 | **Corporate actions are proposed, never imported.** | D2 accepts this by design — a corporate action silently restates history across every account holding the instrument, and `corporate add` previews by default and refuses duplicates precisely to force a human decision before that happens; an importer that wrote proposals directly would bypass those guards for the one input, a broker export, most likely to need them. The cost is real: a user who imports and ignores the section `_print_corporate_action_section` (`cli.py`) prints under `_CORPORATE_ACTION_HEADER` gets an import whose positions in the affected instruments are wrong until the proposals are run by hand, and nothing records that the section was ever shown once the terminal output scrolls away. The mitigation is entirely that the report is loud: printed unconditionally, on both preview and `--commit`, never batched, summarised, or made optional. |
| 43 | **Cash in lieu of fractional shares is recognised but not applied.** | `ImportBatch.cash_in_lieu` (`importers/base.py`) carries the description text for these rows and `_print_corporate_action_section` (`cli.py`) prints them under their own heading, but nothing computes or applies the amount. D6 accepts this deliberately: real money moves and neither the ledger's cash nor its realised P&L reflects it — the same arithmetic gap #35 already tracks for merger cash. This entry cross-references rather than duplicates, checked rather than assumed: gap #35 above is still "Merger cash is not modelled," the same defect one layer up. **Corrected 2026-08-17:** `_print_corporate_action_section`'s own cash-in-lieu message used to cite #35 by number — the gap for *merger* cash, which never mentions fractional shares — rather than this entry, which was opened for exactly this row type. It now cites #43. |
| 44 | **CUSIPs are never stored.** | D7 keeps CUSIP resolution advisory and per-import, and names this explicitly as the tradeoff that let recognition ship without a schema change. `CorporateActionProposal` (`importers/base.py`) carries `source_cusip`/`resulting_cusip` for a single proposal's own report, but nothing writes a CUSIP-to-instrument mapping anywhere, and `instrument` (`db/schema.sql`) has no CUSIP column to write one into. A user who resolves an unmapped CUSIP to a symbol by hand has done that resolution for this import only — the identical unresolved action reappears, unresolved, the next time the same or an overlapping file is imported. |
| 45 | **The `#REOR` grouping is venue-specific and format-fragile.** | `_reor_base` (`importers/fidelity.py`) strips exactly one trailing character from a `#REOR` token — the one character verified, against the real exports, to vary between legs of the same event — and its own docstring warns against widening that without re-verification. Nothing about the grouping generalises past Fidelity's own format for this one reference: a change to how Fidelity renders it degrades every group to `_group_corporate_actions`'s `(ex-date, CUSIP pair)` fallback (`importers/fidelity.py`), which cannot distinguish two distinct actions on one instrument on one date — D4's own fallback, not a bug in it, but a real loss of precision the moment the primary signal disappears. |
| 46 | **Ongoing incremental imports remain unsolved.** | The design's own verification table (§1) found the 90-day Activity & Orders cap does not block backfilling the multi-year History exports this branch covers — both accounts' full history is importable from files already in hand. What it still constrains is keeping the ledger current afterward: Activity & Orders is the only dialect short enough to pull incrementally, and it is also the one dialect that never carries a corporate action — `db/importing.py`'s own routing note is explicit that a row with no `external_ref` is never routed, and only the History dialect's account-less rows fall into that case. A corporate action occurring after the last History export was pulled has no dialect that both reaches it inside 90 days and would recognise it if it did — a different, harder problem than the one this branch closed, not a leftover piece of the same one. |
| 47 | **A spinoff's parent is taken from the row when the row names one, and identified by elimination only when it does not.** Found while implementing, not anticipated by the design's own §10. **Corrected 2026-08-17 (final fix wave) — the original entry's claim about the real accounts was false, and is the reason this entry now describes a different mechanism.** | `_derive_cusip_pair` (`importers/fidelity.py`) can never populate `source_cusip` for a spinoff — its group is always the single row for the child received, and resolving a source needs a negative leg. The parent is therefore not a CUSIP question at all. It is, however, a question the venue's own row usually answers: a Fidelity spinoff row reads `DISTRIBUTION SPINOFF FROM:(TICKER )` with the child in the `Symbol` column, so the parent's ticker is **stated**. `CorporateActionProposal.parent_symbol` (`importers/base.py`) now carries it and `_complete_spinoff_ratio` (`cli.py`) reads that instrument's holding directly.<br><br>**What was wrong.** This entry previously said the sole-LONG-holding inference was "true of the two real accounts checked". It is not, and this was **verified by recomputing the holdings from the real exports**, not doubted on principle: at the only real spinoff's ex-date the account is long many instruments, not one — and it still is when only the file containing the spinoff is imported. The elimination rule therefore returned "ambiguous" on 100% of the real data and the completion never once fired. The stated-parent path resolves the same case exactly. What remains open is narrower: a spinoff row that states **no** parent still falls back to elimination, which is ambiguous whenever the account is long more than one thing; and a stated parent the account is not long (typically because the purchase lives in a year-file that has not been imported) reports and stops rather than substituting some other instrument. Both degrade to `ratio = None` with a note, never to a guess. |

---

## Found while unblocking the importer's non-corporate verbs (2026-08-18)

`feat/importer-blocking-verbs` closes gap #31's remainder except `TRANSFER OF ASSETS
ACAT` (now narrowed above, deferred to branch B): retirement rollover/early-distribution
cash, a plain non-spinoff `DISTRIBUTION` read as a share distribution, and a broker
amendment cluster (original, cancel, correction) netted to one dated fill. Verified by
running `FidelityImporter.parse()` over all eleven real history files: every file now
reports zero blocking rows except the one carrying `TRANSFER OF ASSETS ACAT`, which
blocks on that verb alone. Full design:
[`docs/superpowers/specs/2026-08-18-importer-blocking-verbs-design.md`](superpowers/specs/2026-08-18-importer-blocking-verbs-design.md).
Its own §9 names four gaps this design creates by its own decisions; a fifth was found
only once the netting pass was implemented and traced through `parse()`'s row loop.
Three more (#53-#55) were found by the whole-branch review at the end, and are recorded
here deliberately rather than fixed on this branch: #53 needs a design decision, and
#54/#55 are pre-existing shapes this branch widened rather than introduced.

| # | Gap | Why it matters |
|---|---|---|
| 48 | **The amendment matcher is fitted to one real cluster.** | `_amendment_plan` (`importers/fidelity.py`) says so in its own docstring: "This matcher is fitted to a single real cluster; refusing to act is the failure mode it is allowed to have." Same fragility class as gap #45's `#REOR` grouping — derived from a single observed example. A different amendment shape (a cancel with no correction, a correction that changes quantity rather than price, a multi-leg amendment) is not modelled and falls through to the ordinary unmapped path, where it blocks (D4) rather than being netted. That is the designed failure mode, not a defect, but it is a real limit: exactly one complete cluster exists across all eleven real files, so the matcher has been exercised on one example only. |
| 49 | **A cash-only `DISTRIBUTION` blocks.** | The `share_distribution` rule (`RULES`, `importers/fidelity.py`) matches any plain `DISTRIBUTION` row, but the guard beside it in `parse()` (D5) only completes the split reading when the row's quantity is positive — a zero-or-negative quantity routes to `reject()` instead, with a message naming "no positive quantity" so it stays findable rather than silently reclassified. The cost is real: the first cash-only `DISTRIBUTION` this importer meets refuses the whole import rather than being booked as cash. No real example exists yet to reason a cash-carrying case from, so the guard stays conservative. |
| 50 | **The split reading of a share distribution is a choice, not a derivation.** | `_complete_split_ratio` (`cli.py`) always computes `(held + received) : held` — basis-preservation (D2) — never the taxable-stock-dividend treatment, and nothing in the row itself distinguishes the two; the choice is made once, for every account, not inferred per row. If a distribution was in fact taxable income, the ledger's cost basis and every realised gain on that instrument downstream are wrong in a way nothing in `import` surfaces — there is no warning path for "this ratio's tax treatment is unverified." |
| 51 | **Netting makes the ledger disagree with the broker's row count.** | After D3, one fill in the ledger can represent three rows in the export — a netted original and its cancel, replaced by the correction that survives (see gap #48). `_AmendmentPlan.notes` (`importers/fidelity.py`), surfaced through `parse()`'s `warnings`, is the only place this is recorded. Reconciling a Deadband trade back to a broker statement line-by-line now requires knowing that netting happened, which is why §6 of the design requires the summary to say so rather than leaving it implicit in a lower fill count. |
| 52 | ~~**A suppressed amendment row no longer contributes to `refs_seen`.**~~ — **FIXED 2026-08-18 (final review, T4-a).** | The `continue` for a row in `plan.suppressed`, in `parse()`'s materialised-row loop (`importers/fidelity.py`), used to sit ABOVE the `refs_seen.add(account)` call, so a suppressed original or cancel never reached it — narrowing `ImportBatch.refs_seen`'s own unconditional claim (`importers/base.py`) that every account ref in the RAW rows is included "whether or not the row went on to become a fill or cash movement". It now sits BELOW, so the docstring's rule holds again. **The narrowing was not harmless after all**, which is why it was fixed rather than left recorded: while `_amendment_plan` was account-blind (fixed in the same review, F1), a cancel in one account could suppress an unrelated genuine fill in ANOTHER — and that account then vanished from `refs_seen` too, blinding `db/importing.py`'s unregistered-ref check, the one safety net that would have surfaced it. Reproduced: with both defects present, `test_an_amendment_leg_cannot_net_away_another_accounts_fill` (`tests/test_fidelity.py`) sees one ref where two rows carried two.
| 53 | **The share-distribution ratio is derived from RAW fill quantities, against a holding view that is corporate-action-ADJUSTED.** Found by the final whole-branch review; the design did not anticipate it. **This is the most consequential thing this branch leaves open, and it needs a design decision, not a patch.** | `_long_holdings_as_of` (`cli.py`) sums `fill.quantity` straight out of the `fill` table. That is not what the ledger means by "held": raw fills are never rewritten — `regroup_account` (`db/trades.py`) applies `adjust_fills` (`ledger/corporate.py`) to an in-memory copy and never writes it back, which gap #38 above records — so after any prior split or reverse split on that instrument, the raw sum and the adjusted holding are different numbers. `_complete_split_ratio` computes `(held + received) : held` from the raw one and labels the note "derived from the ledger", with no hedge and no warning. <br><br>**Worked example.** A prior 2:1 split on the instrument: raw held 60, true (adjusted) held 120. This row delivers 40. The completion prints `100:60` → **5:3**; the truth is `160:120` → **4:3**. Both are plausible-looking reduced ratios, and nothing distinguishes them on screen. A human who pastes that `corporate add` stores a wrong split against every account holding the instrument. <br><br>**Not hypothetical.** Two reverse splits appear in the real exports (counted by running `FidelityImporter.parse()` over `imports/`, not estimated). Symbol changes degrade safely here — `open_positions`' `COALESCE(t.effective_instrument_id, f.instrument_id)` resolution means an identity change shows up as "holds no LONG position", which reports and stops. Splits do not degrade safely: they produce a confident wrong number. <br><br>**Why it is not fixed here.** "Held at the ex-date, against adjusted history" is not a query change; it is a decision about what the ledger's holding view IS at import time, before any of this import's own fills exist, and it has to answer whether the received quantity on the row is stated in pre- or post-adjustment units. `_complete_spinoff_ratio` (`cli.py`) reads the same seam and inherits the same defect. Until it is decided, the honest reading of that note is "derived from the raw fill table", not "derived from the ledger". |
| 54 | **Amendment netting is file-scoped, so a cluster split across two year-files can never net — and the file that holds the legs blocks permanently, with no override and nothing saying why.** | `_amendment_plan` (`importers/fidelity.py`) is called once per `parse()`, on one file's rows, so an original in the 2025 export and its `CANCELLED TRADE`/`CORRECTED CONFIRM` legs in the 2026 export are never in the same call and no chain is ever complete. This degrades SAFELY — D4 holds, the legs stay unmapped and money-carrying and block, nothing is guessed — which is why it is recorded rather than fixed. The cost is that the refusal is unactionable: the blocking message says the correction "matched no unique cancelled original", which reads like a data problem the user could fix, when in fact no single file can ever contain the match. There is no override flag and no message distinguishing "this cluster is malformed" from "this cluster spans your import window". <br><br>The one real cluster makes this concrete rather than theoretical: its two legs carry a `Run Date` nineteen days after the original they amend (spec §1.2). Nineteen days is comfortably inside a year, so this cluster happens to land whole — but a December original with January legs is an ordinary shape, not a contrived one, and the year-file boundary is exactly where Fidelity splits these exports. Related to gap #46 (incremental imports), which is the same one-file-at-a-time assumption seen from the other side. |
| 55 | **A corporate-action row with a non-finite quantity is dropped with a warning, not routed through `reject()` — so a money-carrying row neither imports nor blocks.** Pre-existing for every corporate-action kind; this branch widened the set of rows that reach it. | The non-finite-quantity guard in `parse()`'s `Outcome.CORPORATE_ACTION` branch (`importers/fidelity.py`, the check whose message reads "corporate action has non-finite quantity ... excluded from grouping") appends to `warnings` and `continue`s. It never calls `reject()`, so the row reaches neither `unmapped_rows` nor `blocking` — and `reject()`'s own docstring is explicit that routing every dropped row through it is what stops exactly this asymmetry from recurring "the next time a new guard is added". `Decimal("NaN")` and `Decimal("Infinity")` construct without raising, so this is reachable from a malformed export, not only from a corrupted one. <br><br>The exposure is that a `DISTRIBUTION` row carrying a real dollar figure in `Amount ($)` and a garbled `Quantity` vanishes with one warning line, on an import that reports success. This branch's `share_distribution` rule (`RULES`, `importers/fidelity.py`) is what widened it: plain `DISTRIBUTION` rows now reach this branch, where before they were unmapped and blocked. Left unfixed here because changing it changes refusal behaviour for all five pre-existing corporate-action kinds, which is a scope this branch's own acceptance run does not cover. |
| 56 | **The transfer×spinoff interplay is undesigned.** `adjust_transfers` (`ledger/corporate.py`) deliberately skips spinoffs, while `adjust_fills` mints child fills per-fill regardless of later outflows. | A position transferred out before a spinoff's ex-date still has its BUY fills' basis reallocated and child fills minted — the same position-blindness gap #53 records — while the transfer itself is not rescaled or mirrored on the child at all. No real data exercises the combination (one transfer, no spinoff on that instrument); recorded so the first file that does is a design decision, not a silent wrong number. |
| 57 | **`asset_transfer.market_value` is stored but consumed by nothing.** | It is the one number the broker printed on the row; discarding it would be worse. First likely consumer is the trade-detail timeline (read-only API spec's typed event lists). Until then nothing renders it except `transfers list`. |
| 58 | **Manual trades and transfers have no interaction.** Transfers participate only in `regroup_account`'s auto pass; a manual trade's fills are excluded from it, and no manual-grouping path can hold a transfer allocation. | Unreachable today — manual grouping has no db/ entry point (see gap noted in `regroup_account`'s own comment) — but a future Entry & Import milestone that adds one must decide whether a transfer can close a manually-grouped position, or the transfer will raise `TransferError` against a position the auto pass cannot see. |
| 59 | **An account's equity series drops by the transferred market value at the transfer date, which reads as a loss at a glance.** | It is not a loss and the ledger says so (`realized_pnl` is untouched; `qty_transferred` names the cause). Presentation is a UI concern for the dashboard/trade-detail screens; the data is correct. Recorded so the first "why did equity fall here?" question lands on this row instead of a debugging session. |
...
Its own §9 names three of the five gaps below — the split-adjustment gap, the
health check, and the env-var footgun; the other two (#62, #63) were found
while reviewing `add_manual_fills` itself.

| # | Gap | Why it matters |
|---|---|---|
| 60 | ~~**The write instance has no health check.**~~ — RESOLVED (2026-08-27) | There is no longer a separate write instance to miss: the collapse to one service (§6 of the tailnet-identity design) means the deploy script's existing health check on that single service covers both reads and writes. |
| 61 | **`DEADBAND_ENABLE_WRITES` gone wrong is still a silent footgun, though no longer the only thing standing between a shared network and an unauthenticated write surface.** | `create_app()` (`api/app.py`) registers the write routers only when this variable is set. That used to be the sole gate; it no longer is, because every write route also verifies caller identity (`api/identity.py`), a second, independent layer §6 of the design describes — and §6 still spells out why a source-address check can't substitute for either layer (the proxy is the client, so a proxied request already presents as `127.0.0.1`). `tests/api/test_write_gating.py` pins the default — write routes are absent unless the flag is set — but nothing pins the deployed unit file itself; a unit that sets the variable by mistake fails silently, the same way #37 (corporate-action uniqueness) and #28 (unrecognised cash kinds) are silent by omission rather than by error. ~~**The footgun inside the footgun:** the check is `bool(os.environ.get("DEADBAND_ENABLE_WRITES"))`, so any non-empty string reads as enabled — `DEADBAND_ENABLE_WRITES=0` and `DEADBAND_ENABLE_WRITES=false` both turn writes ON, not off.~~ **Closed 2026-09-09.** `create_app` now parses the value: `os.environ.get(...).strip().lower()` against an affirmative set of `1`/`true`/`yes`/`on`, and anything else — `0`, `false`, whitespace, a typo — leaves writes off. Unrecognised values disable rather than raise, because writes are opt-in and a startup exception would take reads down too; a mistyped flag now degrades the instance to read-only, which is visible in the UI immediately. Mutation-tested: restoring the `bool(...)` call turns 10 parametrized cases in `tests/api/test_write_gating.py` red. **The rest of this entry still stands** — nothing pins what the deployed unit file actually carries, so a unit that sets the variable by mistake is still silent, and a mistake there surfaces as 403s from the identity check rather than as an open write surface. |
...
| # | Gap | Why it matters |
|---|---|---|
| 71 | **The snapshot form's ledger comparison is as-of-now, not as-of-statement.** | `db/cash.py`'s `account_cash` and `db/positions.py`'s `open_positions` take no `as_of` — both compute from everything in the ledger at the moment they are called. The Snapshot screen's "ledger, as of now" column is therefore today's figure beside a statement that may be months old. The raw difference is always shown, for every date, and the screen never prints a verdict or a tick of agreement — it only adds a caveat, gated on the statement date not being today, explaining that the difference is not a reconciliation because the ledger side is as-of-now. This is not new to the form — `cli.py`'s `reconcile` has exactly the same property, where `--as-of` selects WHICH snapshot to compare against while the ledger side is always the present. Closing it means historical position and cash reconstruction, which is a feature, not a fix. The comparison still earns its place: a transposed cash/equity pair shows as two large offsetting differences regardless of date, which is the failure `add_snapshot`'s keyword-only signature exists to prevent and the one a human is most likely to commit at the keyboard. |
...
| 74 | ~~**CI runs no lint step at all.**~~ **Closed 2026-09-09** — see the closing note at the end of this row. | Verified 2026-08-28: `grep -n "lint\|ruff\|oxlint" .github/workflows/ci.yml` returns nothing — the workflow runs the pure suite, the DB suite, a zero-skip guard, and `pnpm build`, but never `ruff check` or `pnpm lint`. `uv run ruff check . --statistics` on `main` (commit `dad7fff`) reports 36 findings: 12 `B008`, 9 `I001`, 7 `UP037`, 6 `E501`, 1 `E402`, 1 `F401`. This branch's own tree adds 4 more `B008` findings on top of that (`api/marks.py`, `api/snapshots.py` — the same `Depends()`-in-a-default pattern used throughout `api/`), for 40 total; the two counts are kept separate deliberately, since a bare `ruff check .` from a worktree root picks up that worktree's own new files, not only what is on `main`. `pnpm lint` reports two `react(set-state-in-effect)` warnings (`AccountDetail.tsx:13`, `Trades.tsx:38`). None of it is checked by any gate. This milestone's own implementation is the anecdotal cost of that gap, not a repo-checkable fact: two separate React snippets proposed during this plan each introduced a third `set-state-in-effect` warning, and both were caught only because the dispatch instructions happened to pin an exact expected warning count — nothing in CI would have stopped either from reaching `main`.  **Closed 2026-09-09.** `.github/workflows/ci.yml` now runs `uv run ruff check .` in the python job (before the suites, so a formatting failure costs seconds rather than the DB lane's six minutes) and `pnpm exec oxlint --max-warnings 0` in the web job. Getting to zero took three moves, and the first is the one worth knowing: **all 17 `B008` findings were FastAPI's own `Depends()`/`Form()`/`Query()`-in-a-default idiom**, so they are silenced by `[tool.ruff.lint.flake8-bugbear] extend-immutable-calls` naming those seven callables — NOT by ignoring B008, which would also stop catching what the rule is for. Verified by probe: `def f(t=datetime.now())` still fails, `def f(conn=Depends(...))` does not. The other two: `ruff check --fix` cleared 17 auto-fixable findings (9 `I001`, 7 `UP037`, 1 `F401`), and the remaining 6 `E501`/`E402` were fixed by hand. On the web side, `pnpm lint` exits 0 on warnings and so would have reported without gating; `--max-warnings 0` makes it a gate, and the two pre-existing `set-state-in-effect` warnings are suppressed at their sites with the reasoning inline rather than tolerated by a threshold a future warning could quietly occupy. Both suppressions cover the same deliberate pattern — clearing stale state when the fetch key changes, so a screen shows "loading…" instead of the previous account's numbers under the new account's name. Rewriting those two effects is real behavioural work on daily-use screens with no frontend test runner (gap #73), and did not belong in a lint-gate change. |
| 75 | **`api/fills.py` parsed `executed_at` with a bare `datetime.fromisoformat`.** *Closed on this branch — recorded because the reasoning is worth keeping and because the first version of this entry was wrong about the severity.* | At `api/fills.py:83`, inside the same tuple whose money fields were switched to `api.validation.parse_decimal` earlier on this branch, the timestamp kept a bare `datetime.fromisoformat`. Two bad inputs escaped as uncaught **500s**: a malformed string raised `ValueError` straight out of the handler, and a naive (offset-less) one got as far as `Fill.__post_init__` (`ledger/types.py:139-141`), whose `executed_at must be timezone-aware (UTC)` check refused it — the correct outcome reached by an unusable route, since the caller sees a server error with no field name and the refusal happens after the write pool is already checked out. **An earlier draft of this entry claimed a naive timestamp was silently stored into `TIMESTAMPTZ` as though it were UTC. That was wrong**, and writing the test is what disproved it: `ledger/types.py`'s guard means no naive timestamp ever reaches the database. The defect was a bad error surface, not data corruption — worth recording, because a gap entry that overstates its own severity is the kind of thing that gets trusted and then acted on. Now `parse_instant(leg.executed_at, f"fills[{i}].executed_at")`, refusing both as a 422 that names the offending leg, before the transaction opens. Mutation-tested: restoring the bare call turns both refusal tests red. |
| 76 | **Spec §4 claims this milestone "lights up ... the Accounts screen's rules panel"; it does not, and cannot as designed.** | Verified 2026-08-28: `api/accounts.py:85-89` states plainly that positions are deliberately not valued on that endpoint — "Valuation lives in /api/dashboard and only there: two endpoints pricing the same position independently is two chances to disagree about what it is worth" — and `GET /api/accounts` returns cash, open/closed trade counts, realized P&L and `has_rule`, with no equity, no headroom, no drift figure of any kind. Filling in `mark` and `account_snapshot` rows via these two new forms therefore changes nothing rendered on the Accounts screen; the rules panel reading those figures is a later milestone's work, not this one's. **The Dashboard tiles do light up exactly as promised** — `api/dashboard.py` computes equity, unrealized P&L and drift from marks and snapshots, so that half of the spec's claim is accurate. This is worth recording because the spec, the session handoff, and the plan's own framing all repeat the "Accounts screen" half of the claim; the next person to look at the Accounts screen and find it unchanged will otherwise file that as a regression instead of the documentation error it is. Related smaller delta while in this section: spec §4 also fixes the Entry control at four segments ("Fill · Multi-leg · Import · Marks"; D11), but this branch ships five, splitting Marks and Snapshot into their own tabs rather than one combined "Marks" segment. The split is the better call — `Entry.tsx` was already 812 lines before either was added — and the committed plan for this milestone says so explicitly, but the spec's D11 sentence itself was never updated to match. |
| 77 | **The instrument issue #27 already created is still one merged row, and it blocks the schema-level half of that fix.** | `importers/fidelity.py` no longer merges blank-symbol rows — `derive_symbol` keys them on the ISIN carried in the Description, falling back to the description itself — so nothing NEW collapses. The row already in the ledger is untouched: one instrument with `symbol = ''` holding a multi-year run of fills that belong to four distinct securities, one issuer accounting for three of them through two successive identity changes. It still renders as a nameless row with `unvaluable_reason` NULL. ~~**This is also why issue #27's second suggested guard — `CHECK (btrim(symbol) <> '')` on `instrument` — is NOT in this branch.**~~ **Superseded 2026-09-09: the guard shipped without the repair.** The reasoning above is correct about a *validating* constraint — `ALTER TABLE ... ADD CONSTRAINT` scans existing rows, would fail against the row below, and since `cli.py migrate` runs on every deploy that breaks deploys permanently rather than failing once. What it missed is `NOT VALID`, which skips the scan and enforces on every INSERT and UPDATE from that point on — the entire behaviour #27 asks for. Migration 005 adds it that way, so **a new blank symbol is now refused by the database regardless of this row's fate**. The row stays a known exception until repaired; a later migration then runs `VALIDATE CONSTRAINT` to close it fully, and `test_the_constraint_is_not_validated` is written to fail when that happens so the flip cannot go unnoticed. **The lesson worth keeping: this entry described a blocked state accurately for three weeks and was wrong about it being blocked** — the constraint was reachable the whole time. Repair needs the owner: re-importing the affected exports would recreate the rows correctly but rewrites real ledger history, and splitting the three-ISIN issuer is entangled with a 2022 reorganisation whose ratio and basis allocation are still unanswered. Verified 2026-09-01 against the real exports: 0 blocking rows, and the previously-merged fills resolve to 4 separate instruments. |
| 78 | **A SHORT option expiring is handled by a convention no real row has ever confirmed.** | `importers/fidelity.py` reads an EXPIRED row's `Quantity` as the position DELTA: negative means contracts left, closing a long (a SELL); positive means they came back, closing a short (a BUY). The first half is solid — verified 2026-09-06 against every EXPIRED row in five years of real exports, 27 of 27 negative against a position the account's own BOUGHT/SOLD rows show is long. The second half is **reasoned, not observed**: the data holds 133 long option opens and exactly one short, and that short was closed by buying it back rather than by expiring, so no short has ever expired. If Fidelity in fact writes a negative quantity on EVERY expiry regardless of direction, a short expiry will be booked as a further SELL and the position will silently grow instead of closing — the same failure mode this entry's own fix corrected for longs. The owner chose this over blocking such an import (the alternative, and what decision E2 does for assignment and exercise). The first real short expiry is what will settle it; `tests/fixtures/fidelity/real_shape_activity.csv` carries a synthetic positive-quantity row and a comment saying not to "correct" it without one. **Note also that the design doc's own empirical claim — `Quantity` is "negative for a short position, positive for a long one" — is false, and the spec, the implementation and two tests all encoded it together, so nothing caught it for three weeks.** |

---

## Found while adding a quote source for marks (2026-09-16)

Subsystem D's first slice: `GET /api/quotes` proposes prices and the marks
screen pre-fills from them. Three limits are deliberate and each reads like an
oversight from the screen itself.

| # | Gap | Why it matters |
|---|---|---|
| 79 | **A mark does not record when the provider said its price was true.** | `mark`'s primary key is `(instrument_id, as_of)` and the table has no `quoted_at` column, so the only instant stored is the `as_of` the user picked — their valuation moment, not the quote's. `marketdata.types.Quote` carries a `quoted_at` and Yahoo's `fast_info` does not populate it, so today it is always `None` and nothing is lost; the gap is what happens when a provider that *does* report one is added. A mark fetched from a quote that was six hours stale is then indistinguishable in the database from one fetched live, and the difference is exactly what decides whether an unrealized P&L figure means anything. Deliberately not fixed here: it is a schema change plus a migration, and smuggling one into a feature branch is how the two get reviewed as a single thing. The screen shows quote provenance at review time, which is the mitigation and is weaker than storage — it depends on someone looking. |
| 80 | **`yfinance` scrapes an endpoint Yahoo does not document, and this repo now depends on it at import time of `api/app.py`.** | `create_app` constructs `YahooQuotes()` unconditionally, so the package is a hard dependency of the API process even for a deployment that never fetches a quote. That is cheap today (the constructor does no I/O and the `yfinance` import is deferred into `_live_fetch`), but the dependency is real and it is the fragile kind: yfinance breaks when Yahoo changes a page, the same way `yt-dlp` does on this host. `QuoteSource` is what keeps the blast radius to one file. What is NOT covered anywhere: nothing in CI or the suite detects that Yahoo has changed and the provider has silently stopped resolving symbols — every provider test runs against recorded fixtures (spec §9, deliberately), so the first signal is a refresh that returns everything `unquoted`. That is visible on the screen rather than silent, which is the reason it is acceptable. |
| 81 | **"Still the fetched price" is decided by string equality on the input.** | `Marks.tsx` compares the trimmed input against the price the provider returned to decide whether to submit `source: "yahoo"` or let it default to `manual`. Re-typing the identical digits therefore still records the provider, and typing `12.340` where the provider said `12.3400` records manual — both are the honest-ish answer to a question the UI cannot really ask, since it does not track keystrokes. The consequence is bounded: the field is provenance, not a value anything computes from, and both failure directions leave a mark whose price is exactly what the user accepted. Recorded because the next person to read that comparison will wonder whether it was considered. |
