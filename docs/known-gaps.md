...
- **No schema counterpart to the `resulting_instrument_id != instrument_id` guard.**
  ...
  corporate action in Python, but `db/schema.sql:198-211`'s `corporate_action` table has no
...
| # | Gap | Why it matters |
|---|---|---|
| 1 | ~~`upsert_instrument` repaints only `symbol`; a wrong `contract_multiplier`, `strike` or `expiry` stored on first insert is never corrected~~ — FIXED 2026-08-09. `upsert_instrument` (`db/instruments.py:14,56-67`) now repaints `contract_multiplier`, `root`, `chain` and `contract_address` from `EXCLUDED` on every upsert, not just `symbol`. **Correction to this entry's own claim:** `strike` and `expiry` were never actually at risk — for an option both sit *inside* `instrument_natural_key` (`ledger/types.py:127-144`), so a different value mints a different row instead of drifting on an existing one. `contract_multiplier` was the only field outside the key that could go stale, and it is now the one fixed. | Repainting has a cost of its own — see the new gap below. |
| 2 | ~~No `CHECK (contract_multiplier > 0)` or `CHECK (mark.price >= 0)`~~ — already closed, not by this plan. `instrument_multiplier_chk` (`db/schema.sql:53-54`) and `mark_price_chk` (`db/schema.sql:192-193`) both exist. | Stale entry: the constraints were already shipped when this was last written down. |
...
| 5 | ~~`fill.updated_at` is never written (no triggers)~~ — already closed, not by this plan. `set_updated_at()` (`db/schema.sql:224-228`) and `fill_set_updated_at` (`:231-234`) both exist. **This strikes only "does the trigger exist."** A separate, still-open item — that the trigger has no *behavioural* test, i.e. nothing proves it actually fires — is recorded under "Carried into A-2 part 2 and beyond" below, and striking this row does not touch it. | |
...
| 7 | ~~Preview cannot report duplicates (spec §7 requires it)~~ — already closed, not by this plan. `--check-duplicates` (pre-flight routing guard `cli.py:188-209`, the reporting that actually fulfils §7 at `:254-262`, flag registered `:1281`) shipped in PR #3. | Stale entry, same shape as #2 and #5. |
...
| 22 | `reconcile` refuses an unknown account id while `positions` does not (spec §10.5) | Deliberate here — reconcile's whole purpose is to be trustworthy about absence (`cli.py:1048-1055`) — but it leaves the two commands inconsistent: `positions` (`cli.py:637-649`) prints "no open positions" (`:727`) for a well-formed but nonexistent UUID rather than refusing. The inconsistency should be resolved by making `positions` stricter, not by loosening `reconcile`. **Subsumed and extended by #26 below** — the spread was three behaviours when #26 was written; `snapshot add`'s raw traceback has since been fixed, so it is back to the two this row names. |
...
time inside `regroup_account` (`db/trades.py:83-104`), and `deadband corporate
add`/`list`/`remove` (`cli.py`) all shipped in this branch — for `split` and
`reverse_split` only. The other three `ActionType` members are computed correctly by
`ledger/corporate.py` and **refused by the CLI**; see gap #39, which is the reason the
design's C3 ("all five action types are supported by the CLI") does not describe what
shipped. Full design:
...
| 34 | **Manual trades are not split-adjusted.** | A fill wholly owned by a manual trade is excluded from `regroup_account`'s auto pass — and therefore from `adjust_fills` — before corporate actions are ever applied. `manual_held` computes how much of each fill a manual trade already holds, and a fill whose remaining quantity is exhausted is dropped from the pass entirely (`db/trades.py:60-81`); the code's own comment names the consequence directly: "fills WHOLLY owned by a manual trade never reach this point... so manual groupings are not split-adjusted" (`db/trades.py:96-98`). A user who hand-groups a trade and then imports a stock split finds that trade's quantity frozen at the pre-split value forever, silently disagreeing with every auto-grouped trade on the same instrument. Fixing it means deciding what a permanent user grouping means across a restatement — does a manual trade's quantity get rewritten at read time the same way a fill's does, or does "manual" mean "frozen as entered," by design? — and that decision was kept out of scope here. |
| 35 | **Merger cash is not modelled.** | `CorporateAction.cash_component` (`ledger/corporate.py:48`) is stored — `add_action` (`db/corporate.py:75-96`) will happily persist whatever a caller constructs, though the CLI does not even expose a flag to set it — and never read: `adjust_fills`'s `MERGER` branch (`ledger/corporate.py:184-194`) scales quantity and price by the stored ratio and touches nothing else. A merger paying part cash, part stock — the common real shape — understates the cash received by exactly that amount, with no warning, and the field's mere existence in both the dataclass and the schema invites the assumption that recording it does something. |
| 36 | **No audit trail on restatement — what changed, only that something did.** | `corporate_action` does carry `created_at` (`db/schema.sql:210`), so *when* a row was inserted is recorded — but `remove_action` (`db/corporate.py:114-118`) is a hard `DELETE`, so removing an action erases that fact along with the row itself, and `corporate list`'s rendering (`cli.py:1664-1675`) doesn't print `created_at` even while the row still exists. More basically, nothing records *what* an action changed: `_print_effect` (`cli.py:1373-1399`) prints the before/after diff once, to stdout, at the moment of `add`/`remove`, and that is the only place the diff is ever visible. This is the same shortcoming gap #15 records for `contract_multiplier` repainting, where the next upsert silently revalues every historical fill with no record of the old value, the new value, or when the change happened — here it recurs one layer up, at the CLI rather than the storage layer. Once the terminal output scrolls away, the only way to reconstruct what a since-modified or since-removed action did to a position is to rebuild the state by hand. |
| 37 | **No database-level uniqueness** on `(instrument_id, ex_date, action_type)`. | `find_duplicate` (`db/corporate.py:121-142`) is the only thing standing between a double keypress and a silently wrong position — its own docstring says so: "There is no UNIQUE constraint on the table... so this is an application-level guard." `corporate_action` (`db/schema.sql:198-211`) carries `CHECK` constraints on its ratio columns but no uniqueness constraint at all; direct SQL, a future importer, or a bug in `cmd_corporate_add` that skips the check could still double-enter the same split and silently double-adjust every fill it touches. Adding the constraint is a migration and was kept out of scope. |
| 38 | **An action recorded against the *result* of an earlier action regroups nothing.** | Both the CLI's `--commit` path (`_regroup_holders`, `cli.py:1415-1420`) and `preview_effect` (`db/corporate.py:181-186`) scope the affected accounts by `fill WHERE instrument_id = <the action's own instrument>`. Because raw fills are never rewritten — `regroup_account` applies `adjust_fills` to an in-memory copy and "never [writes it] back to the fill table" (`db/trades.py:83-87`) — a symbol change from one instrument to another leaves every fill permanently carrying the *source* instrument's id. A later action recorded against the *resulting* instrument therefore matches no fills: the preview reports that nothing is affected and no account is regrouped, while the stored action does genuinely move positions the next time anything else triggers a regroup on that account. Preview and commit agree with each other, so the inconsistency is silent rather than contradictory — there is no error, and no screen where the two disagree. <br><br>**The same root cause has a second half (added 2026-08-15, final review).** "Raw fills are never rewritten" does not only misroute a *later* action — it also misroutes the *position*. `open_positions` resolves a position's instrument from the RAW opening fill (`LEFT JOIN fill f ON f.id = t.opening_fill_id` → `f.instrument_id`, `db/positions.py:36-37`), while `regroup_account` writes `trade.primary_underlying` from the ADJUSTED fill (`db/trades.py:115-124`, bound at `:178`). An action that *moves* a fill to a different instrument therefore leaves the position reporting under the **old** instrument while `deadband trades` reports the new one, and a mark set on the new symbol never prices the position at all — it is keyed to the source instrument's id, so `latest_marks` looks up a row that will never exist and the position renders unmarked. That is a wrong answer on the two screens a user actually reads, not merely a regroup that does nothing, and it is why `corporate add` now refuses the three identity-changing types outright. Recorded in full as gap #39 below.<br><br>**Reachability, after that refusal.** Every type that can carry a `resulting_instrument_id` is now refused by `corporate add`, and `--resulting-symbol` is refused for the two types that remain — so no new `corporate_action` row written *through the CLI* can name a resulting instrument at all, and the scoping half of this row is no longer reachable by typing. It is kept rather than struck because `db.corporate.add_action` will still persist such a row for any other caller, direct SQL will, and nothing rewrites or reports one that is already stored from before the refusal existed. |
| 39 | **Identity-changing actions (merger, spinoff, symbol change) cannot be materialised into `trade`, and the CLI refuses them.** | The engine is not the defect: `ledger/corporate.py` computes all five types, `tests/test_corporate.py` pins all five, and `db/corporate.py` will persist any of them. What cannot happen is turning the last three into `trade` rows, and the reason is the design's own C1 — adjustments are derived at READ time over a `fill` table that is deliberately never rewritten, which is exactly what makes `corporate remove` a genuine undo. That is compatible with an action that rescales a fill's *magnitude* and incompatible with one that changes its *identity*, in two different ways: **(a) merger and symbol change report the position under the OLD symbol** — see the second half of gap #38 above for the mechanism; verified against the real test database, each scenario in its own rolled-back transaction, both types reporting the source symbol at the pre-action quantity. **(b) a spinoff cannot be persisted at all** — its synthetic child fill is given a `uuid5` id (`ledger/corporate.py:32-37`, `:215`) with no row in `fill`, and `trade.opening_fill_id` / `trade_fill.fill_id` are non-deferrable COMPOSITE foreign keys into `fill (id, account_id)` (`db/schema.sql:142-144`, `:164-165`), so the regroup raises `ForeignKeyViolationError` on `trade_opening_fill_fk`. This is a **spec** defect rather than an implementation one: C3 ("all five action types are supported by the CLI") justified itself on the grounds that "the marginal cost is argument validation, not domain logic", and that is empirically false for these three — C1 and C3 are incompatible for any action that moves a fill to a different instrument or creates a new one. So `cmd_corporate_add` refuses `--type merger`, `--type spinoff` and `--type symbol_change`, exits 2, writes nothing and opens no connection (`cli.py:1435-1456`). All five members deliberately stay in argparse's `choices`: the refusal has to carry the reasoning above rather than argparse's bare "invalid choice", because a user with a real merger to record needs to know this is a recorded limitation and not a bug. `split` and `reverse_split` only rescale `quantity`/`price` on a row that already exists and round-trip correctly. Closing this needs a **schema change**, which is why it is a gap and not a fix: an `effective_instrument_id` column on `trade` (written from the adjusted fill, falling back to the raw one) with `open_positions` preferring it over the join, plus spinoff fills actually persisted rather than synthesised on every read — which itself needs a decision about what `corporate remove` then does to fills it wrote. That is larger than the branch that produced this row, and it must not be attempted by widening the refusal's edges: the three types are refused *whole* precisely so nothing half-works. |
