...
| 75 | **`api/fills.py` still parses `executed_at` with a bare `datetime.fromisoformat`.** | At `api/fills.py:83`, inside the same tuple whose money fields (`price`, `fee`) were switched to `api.validation.parse_decimal` on this branch. An unparseable string raises `ValueError` out of the handler as an uncaught **500**, not a clean 422 — the same class of hazard `parse_decimal` closed for the money fields, left open here. Worse, a naive (offset-less) timestamp is *accepted* and stored into a `TIMESTAMPTZ` column as though it were UTC, the same silent wall-clock misinterpretation `web/src/datetime.ts` documents at length for the browser side of this codebase. `api.validation.parse_instant`, added on this branch for `POST /api/marks`, is a one-line fix here too: `parse_instant(leg.executed_at, f"fills[{i}].executed_at")`. **Deliberately not applied in this fix wave**: `POST /api/fills` is a write route in daily use, and swapping the parser changes its behaviour — a naive timestamp that is silently accepted today would start returning 422. That is a real behaviour change for existing callers/scripts and is a decision for the owner, not something to fold into an unrelated marks-and-snapshots fix wave. |
