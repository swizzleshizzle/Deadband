...
| 2 | ~~No `CHECK (contract_multiplier > 0)` or `CHECK (mark.price >= 0)`~~ — already closed, not by this plan. `instrument_multiplier_chk` (`db/schema.sql:53-55`) and `mark_price_chk` (`db/schema.sql:192-193`) both exist. | Stale entry: the constraints were already shipped when this was last written down. |
...
| 7 | ~~Preview cannot report duplicates (spec §7 requires it)~~ — already closed, not by this plan. `--check-duplicates` (`cli.py:188-209`, flag registered `:622`) shipped in PR #3. | Stale entry, same shape as #2 and #5. |
