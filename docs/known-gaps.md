...
| 43 | **Cash in lieu of fractional shares is recognised but not applied.** | `ImportBatch.cash_in_lieu` (`importers/base.py`) carries the description text for these rows and `_print_corporate_action_section` (`cli.py`) prints them under their own heading, but nothing computes or applies the amount. D6 accepts this deliberately: real money moves and neither the ledger's cash nor its realised P&L reflects it — the same arithmetic gap #35 already tracks for merger cash, which `_print_corporate_action_section`'s own cash-in-lieu message cites by number directly rather than opening a new one. This entry cross-references rather than duplicates for the same reason, checked rather than assumed: gap #35 above is still "Merger cash is not modelled," the same defect one layer up. |
...
| 47 | **A spinoff's parent instrument is identified by elimination, not by a field the export supplies.** Found while implementing, not anticipated by the design's own §10. | `_complete_spinoff_ratio` (`cli.py`) has nothing on `CorporateActionProposal` naming which instrument is the parent — a spinoff's group is always the single row for the child received, so `_derive_cusip_pair` (`importers/fidelity.py`) can never populate `source_cusip` for it, and the proposal carries no symbol field for the parent at all. The code resolves the parent instead as the account's sole LONG net holding as of the ex-date, across every instrument it has ever traded, and refuses honestly — `ratio` stays `None`, with an explanatory note — when that holding is not unique: zero long positions, or more than one, both count as ambiguous rather than guessed at. That is an inference from the account's own trading history, not a fact the row itself supplies, and it holds only as long as an account never carries two long positions through the same spinoff's ex-date — true of the two real accounts checked, not something the code can verify or the export can rule out in general. |
