...
| 25 | `cash_movement.amount` has no `CHECK (amount > 0)` | `OUTFLOW_KINDS`'s docstring (`importers/base.py:45-53`) and `CashMovementRow`'s (`ledger/cash.py:17-19`) both state that `amount` is always positive and that direction lives entirely in `kind` — but nothing enforces it at the database level (`db/schema.sql:177`, a bare `NUMERIC NOT NULL`). A negative stored amount would silently flip a movement's direction inside `net_cash` (`ledger/cash.py:51`) rather than raise. A DB `CHECK` is the sounder fix than a read-side guard, since the convention is meant to hold for every writer, not just this one reader. |
| 26 | Three different behaviours for an unknown account id | `reconcile` refuses cleanly, exit 2 (deliberate, spec §8; `cli.py:996-1003`). `positions` prints "no open positions" (`cli.py:727`). **`snapshot add` lets an `asyncpg.ForeignKeyViolationError` escape as a raw traceback** — `cmd_snapshot_add` (`cli.py:833-926`) never checks the account exists before `add_snapshot`'s `INSERT` (`db/snapshots.py:27-39`) hits `account_snapshot.account_id`'s foreign key (`db/schema.sql:215`), and `main()` catches only `OSError` (`cli.py:1281-1285`) — while the sibling `cmd_regroup` catches `UnknownAccountError` for the identical situation (`cli.py:551`, raised by `db/trades.py:32`). The raw traceback is worse than either of the other two commands' behaviour. This subsumes and extends #22 above — cross-reference the two rather than treating them as unrelated. |
